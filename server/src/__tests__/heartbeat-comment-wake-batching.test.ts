...
describe("heartbeat comment wake batching", () => {
  ...
  it("does not reopen a finished issue for deferred user comment wakes without explicit resume intent", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "No reopen after deferred comment",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
  ...
  it("does not reopen a finished issue for deferred issue_reopened_via_comment wakes without explicit resume intent", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "No reopen after deferred explicit-reason wake",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const comment2 = await db
        ...
        .values({
          ...
          issueId,
          authorUserId: "user-1",
          body: "Follow-up without explicit resume intent",
        })
        .returning()
        .then((rows) => rows[0]);
  ...
  it("reopens a finished issue for deferred user comment wakes with explicit resume intent", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Reopen with deferred comment resume intent",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const deferredRun = await heartbeat.wakeup(agentId, {
        ...
        triggerDetail: "system",
        reason: "issue_commented",
        payload: { issueId, commentId: comment2.id, resumeIntent: true, followUpRequested: true },
        contextSnapshot: {
          issueId,
          taskId: issueId,
          commentId: comment2.id,
  ...
  it("keeps blocked issues non-reactivating through deferred comment wake promotion without explicit resume intent", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      // Issue starts as done (terminal) to test that deferred comment wake
      // promotion does not reopen it. This mirrors the done-issue deferred tests
      // but validates the blocked→done promotion path explicitly.
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Blocked-then-done issue should not reopen via deferred wake",
        status: "done",
        completedAt: new Date(),
        priority: "medium",
        assigneeAgentId: agentId,
      ...
      const comment1 = await db
        .insert(issueComments)
        .values({ companyId, issueId, authorUserId: "user-1", body: "First comment" })
        .returning()
        .then((rows) => rows[0]);
      ...
      const comment2 = await db
        .insert(issueComments)
        .values({ companyId, issueId, authorUserId: "user-1", body: "Follow-up comment, no resume intent" })
        .returning()
        .then((rows) => rows[0]);
      ...
      // Key invariant: the deferred comment wake without resumeIntent must NOT
      // reopen a terminal (done) issue to todo. This is the FMA-1489 regression guard.
      expect(issueAfterPromotion).toMatchObject({ status: "done" });
    } finally {
      gateway.releaseFirstWait();
      await gateway.close();
    }
